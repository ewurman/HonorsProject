...
def worker_behaviour(unit):
    ...
    if location.is_on_map():
        nearby = gc.sense_nearby_units(x, 2)
        for other in nearby:
...
def switch_phase():
    global phase
    if phase == 0 and units_earth[bc.UnitType.Worker] > 8:
        print("Phase 1 starting")
        phase = 1
    elif phase == 1 and units_earth[bc.UnitType.Factory] >= 7:
        print("Phase 2 starting")
        phase = 2
    elif phase == 2 and units_earth[bc.UnitType.Knight] >= 8:
        print("Phase 3 starting")
        phase = 3
    elif phase == 3 and gc.round() > 350:
